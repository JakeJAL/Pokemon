...
import re
import json
from modules.prof_oak_ai import professor_oak_query
...
def price():
    ...
    try:
        ...
        csv_path = os.path.join(script_dir, 'data', 'pokemon_cards_database.csv')
        ...
        with open(csv_path, mode='r', encoding='utf-8') as file:
            ...
            for row in reader:
                ...
                try:
                    ...
                    # Set non-numeric prices to infinity so they appear last
                    row['price_float'] = float('inf')
    ...
    items.sort(key=lambda x: x.get('price_float', float('inf')), reverse=reverse_sort)
...
def load_pokemon_names():
    ...
    if POKEMON_NAMES is None:
        try:
            cards_file_path = os.path.join(os.path.dirname(__file__), 'data', 'all_cards.json')
